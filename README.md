...
- **Every model, every reasoning level** -- Fable 5, Opus 4.8, Sonnet 4.6, GPT 5.4, GPT 5.5, GPT 5.6 Terra, Gemini 3.1 Pro, Gemini 3 Flash, and Kimi K2.6 are registered as Factory custom models with their full set of native reasoning levels. Reasoning effort is chosen per session from Droid CLI's model selector and forwarded upstream unchanged.
- **Fast Mode** -- Optional `service_tier=priority` for GPT 5.4, GPT 5.5, and GPT 5.6 Terra, toggled from the Settings window for lower-latency responses on the OpenAI Responses API.

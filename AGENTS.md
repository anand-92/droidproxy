...
| `src/Sources/ThinkingProxy.swift` | Raw TCP HTTP proxy that forwards requests to CLIProxyAPI (and TLS-forwards Cursor/Junie/Grok). Rewrites the Anthropic-Beta header to drop `redact-thinking-2026-02-12` on Claude thinking requests, injects `service_tier=priority` on enabled Codex fast-mode models, rewrites OAuth Code Assist Gemini `/v1/responses` to `/v1/chat/completions`, sanitizes Grok tool types before `api.x.ai`, diverts `grok-4.6` to the Cursor API as `grok-4.6-fast` when Grok Fast Mode is on, and emits a `REQUEST REASONING` log line per request. Does not inject reasoning or thinking fields. |

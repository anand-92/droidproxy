...
### Added
- **GPT Image skill** -- Bundled `skills/gpt-image` posts `gpt-image-2` (or `gpt-image-1.5`) to `POST http://localhost:8317/v1/images/generations` and `/v1/images/edits` using Codex OAuth. No `OPENAI_API_KEY`. Requires ChatGPT Plus/Pro; image gen is separate from GPT chat models, same as Grok Imagine.

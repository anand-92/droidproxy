...
- **Grok 4.5 OAuth** -- SuperGrok / X Premium+ device-flow login against `auth.x.ai`, credentials at `~/.cli-proxy-api/grok-cli.json`, and TLS forwarding to `api.x.ai` for `grok-4.5`. Connect from Settings → Grok, Apply Factory models, then select `/model` → DroidProxy: Grok 4.5. Some SuperGrok tiers may still require Factory BYOK (`XAI_API_KEY`).

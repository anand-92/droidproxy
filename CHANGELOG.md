...
### Added
- **Grok 4.5 OAuth** -- SuperGrok / X Premium+ device-flow login against `auth.x.ai`, credentials at `~/.cli-proxy-api/grok-cli.json`, and TLS forwarding to `api.x.ai` for `grok-4.5`, `grok-4.3`, and `grok-build-0.1`. No `XAI_API_KEY` required. Connect from Settings → Grok, then Apply Factory models (or `/model` → DroidProxy: Grok 4.5).
